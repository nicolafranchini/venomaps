...
WP OpenMaps is a simple and lightweight plugin to quickly create custom Maps.

== Description ==

This plugin creates custom Maps which can be placed anywhere in your WordPress website: Pages, Posts, Widgets. Upload your custom markers, select a size, set the map's and the markers' coordinates, select a custom style for the map, insert some html content for the popup info boxes and publish your map. The plugin settings page also offers some geolocation utilities to get the coordinates typing an address, or placing the marker in a specific position on the map.
...
* **Settings page with geolocation utility**
* **Responsive design**
...
Use the WP OpenMaps widget to select a map and insert it inside your widget area
...
`<?php echo do_shortcode('[openmap id="xxx"]'); ?>`
replacing the `[openmap id="xxx"]` with the shortcode generated by the plugin inside the map's page. 

== Screenshots ==
 
1. screenshot-1.(png|jpg|jpeg|gif). Default Map
2. Custom map with markers
3. InfoBox windows
4. Map page settings
5. General settings and Utilities
6. Widgets
...
Copy the shortcode from the field below the Map's title and paste it anywhere inside your posts or pages, that's it.

### Adding into widget

Under Appearance > Widgets you will find a new widget **WP OpenMaps**. Add it to your sidebar, select the desired map from the dropdown menu and save.
